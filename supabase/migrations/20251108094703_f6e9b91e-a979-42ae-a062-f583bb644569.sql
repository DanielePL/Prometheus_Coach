-- Add user_world_clock_timezones table for customizable world clock
CREATE TABLE IF NOT EXISTS public.user_world_clock_timezones (
...
-- Enable RLS
ALTER TABLE public.user_world_clock_timezones ENABLE ROW LEVEL SECURITY;

-- RLS Policies
CREATE POLICY "Users can view their own world clock timezones"
  ON public.user_world_clock_timezones
  FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own world clock timezones"
  ON public.user_world_clock_timezones
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own world clock timezones"
  ON public.user_world_clock_timezones
  FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own world clock timezones"
  ON public.user_world_clock_timezones
  FOR DELETE
  USING (auth.uid() = user_id);

-- Update the notification trigger to only send messages for parent recurring events
...
BEGIN
  ...
  IF NEW.assigned_to IS NOT NULL 
    ...
    IF NEW.is_recurring THEN
      v_event_details := v_event_details || E'\n\n' || '🔁 This is a recurring event (' || NEW.recurrence_pattern || ')';
    END IF;
    
    -- Add event ID for linking
    v_event_details := v_event_details || E'\n\n' || 'Event ID: ' || NEW.id::text;
